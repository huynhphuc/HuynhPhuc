...
public class Main {
	...
	public static boolean functionHillClimbing(int aCore, boolean checkBlankFrontOfGoal) {
		/*Try to choose a state has larger a number of blank tiles than the old state.*/
		int newCore;
		if (checkBlankFrontOfGoal) newCore = aCore + 1;
		else newCore = aCore - 1;
		float funcHillClimbing = newCore - aCore;
		...
		while (!stack.empty()) {
			...
			for (int i = 0; i < currentState.getNumTiles(); i++) {
				...
				if (stateRight.moveRight(i) && !stateRight.checkVisitedState(listStateVisited)) 
					if (functionHillClimbing(core, stateRight.checkBlankFrontOfGoal())) {
				...
				if (stateLeft.moveLeft(i) && !stateLeft.checkVisitedState(listStateVisited)) 
					if (functionHillClimbing(core, stateLeft.checkBlankFrontOfGoal())) {
				...
				if (stateUp.moveUp(i) && !stateUp.checkVisitedState(listStateVisited))
					if (functionHillClimbing(core, stateUp.checkBlankFrontOfGoal())) {
						stack.push(stateUp);
						break;
					}
				...
				if (stateDown.moveDown(i) && !stateDown.checkVisitedState(listStateVisited)) 
					if (functionHillClimbing(core, stateDown.checkBlankFrontOfGoal())) {
